...
import { AsyncContent, Dialog, Grid, Button, SelectField, InputField, ModalContext } from '@8base/boost';
...
const ProductEditDialog = ehnhancer(
  class ProductEditDialog extends React.PureComponent {
    ...
    renderForm = ({ args }) => {
      return (
        <Query query={PRODUCT_QUERY} variables={{ id: args.initalValues.id }}>
          {({ data, loading }) => (
            <FormLogic
              type="UPDATE"
              tableSchemaName="Products"
              onSubmit={this.updateOnSubmit(args.initalValues.id)}
              initialValues={args.initalValues}
              formatRelationToIds
            >
